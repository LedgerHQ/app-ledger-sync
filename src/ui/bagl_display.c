...
// FLOW to display add member callback screens:
...
UX_STEP_CB(ux_display_member_rejected_step,
           bnnn_paging,
           ui_menu_main(),
...
UX_STEP_CB(ux_display_seed_id_cb_cancel_step,
           bnnn_paging,
           ui_menu_main(),
...
UX_STEP_NOCB(ux_display_seed_id_log_in_step,
             pnn,
             {&C_app_16px, "Login request", "for Ledger Sync"});
...
UX_STEP_CB(ux_display_update_rejected_step,
           bnnn_paging,
           ui_menu_main(),
           {.title = "", .text = "Operation cancelled"});
...
UX_STEP_NOCB(ux_display_update_trust_step,
             nnn,
             {"Remove existing", "instances to re-add", "those you-ll keep"});
